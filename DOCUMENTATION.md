...
The pre-built bundle at `dist/MangoLiquidUI.lua` (~14,600+ lines) is generated by `build.sh`. Rebuild after modifying any library source file:
...
**What the build does:**
1. Reads all 38+ `.luau` source files in dependency-safe order
2. Wraps each module in a closure with internal `_require()` lookups (replaces `require(script.Parent.X)`)
3. Strips `--!strict` annotations
4. Appends a footer wiring up the library table, short aliases, theme shortcuts, protection aliases, `gui()` helper, `transitionTheme()`, and auto-play intro

**Module processing order** (dependency-safe):

| Position | Module | Key Dependencies |
|----------|--------|-----------------|
| 1 | `Types` | None (shared type definitions) |
| 2 | `MangoProtection` | `Types` only — all other modules depend on this |
| 3 | `Themes` | `Types` |
| 4 | `LiquidFusion` | `Types`, `Themes` |
| 5 | `RefractionProxy` | `Types`, `Themes`, `MangoProtection` |
| 6 | `MangoGlassFrame` | `Types`, `Themes`, `MangoProtection` |
| 7-38 | All other components | `Types`, `Themes`, `MangoProtection`, some compose `MangoGlassFrame` |

**Adding a new module:**
1. Create the `.luau` source file in `MangoLiquidUI/`
2. Add the module name to the `MODULES` array in `build.sh` at the correct dependency position
3. Add `_require()` call, library table entry, and short alias in the `FOOTER` section of `build.sh`
4. Add the type definition to `Types.luau` if it has config/return types
5. Run `bash build.sh` to regenerate `dist/MangoLiquidUI.lua`

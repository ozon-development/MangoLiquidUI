...
- **isDestroyed guards**: MangoSlider and MangoToggle use `isDestroyed` flags to guard against post-destroy callbacks (e.g. `UserInputService.InputChanged`, `animateToState()`) and double-destroy. MangoSlider also sets `isDragging = false` on destroy.

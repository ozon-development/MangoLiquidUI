...
- **Dropdown position tracking**: While panel is open, tracks trigger's `AbsolutePosition` via `GetPropertyChangedSignal` so the dropdown panel follows window drag/scroll movements. Connection disconnected on close.

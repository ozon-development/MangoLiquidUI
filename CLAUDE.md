...
```
Container (Frame, ClipsDescendants=true, height tweens on open/close to push siblings)
  +-- TriggerGlass (MangoGlassFrame, LightweightMode, pill, full-width × triggerHeight)
  |     +-- SelectedLabel (TextLabel, left-aligned text)
  |     +-- ChevronWrapper (Frame, Rotation animates 0↔180 on open/close)
  |     |     +-- ChevronLabel (TextLabel, "v", GothamMedium 10)
  |     +-- HitArea (TextButton, ZIndex=100)
  +-- PanelGlass (MangoGlassFrame, CornerRadius=12, ShadowEnabled=false, below trigger)
        +-- ScrollingFrame (if items > 5, ScrollBarThickness=4)
              +-- UIPadding (4px top/bottom)
              +-- UIListLayout (Vertical)
              +-- ItemN (Frame, 36px, UICorner=8, per-item hover highlight + HitArea)
```
        ...
        +-- ClipFrame (Frame, ClipsDescendants=true, centered, 14px edge padding)
              +-- TrackFrame (Frame, holds all icons, animates Y position vertically)
                    +-- Icon_N (Frame, per-tab icon container, AnchorPoint=0.5,0)
                          ...
                          +-- IconShadow (Frame, accent-tinted, 84% transparent, 14px radius, ICON_SIZE+6)
                          +-- IconBg (Frame, 40% accent fill, 12px squircle)
                          |     +-- UIGradient (IconBgGradient, Rotation=135 diagonal, lighten 0.15 / darken 0.08)
                          |     +-- IconHighlight (Frame, 35% height, white 78% transparent, 12px corner, inner glow)
                          |     |     +-- UIGradient (HighlightGradient, Rotation=90)
                          |     +-- InnerEdgeFrame (Frame, full size, transparent, ZIndex=2)
                          |     |     +-- UICorner (12px)
                          |     |     +-- UIStroke (InnerEdgeStroke, 1px, Border mode, white)
                          |     |           +-- UIGradient (InnerEdgeGradient, Rotation=90, 3-keypoint: 0→0.55, 0.5→0.82, 1→0.95)
...
- **MangoToggle.luau** — Apple-style toggle switch. Pill-shaped track with spring-animated knob (0.25s Back Out), color-transitioning track (0.2s Quad). Theme-aware. `isDestroyed` flag guards `InputBegan` handler, `SetState()`, `animateToState()`, and `Destroy()` against post-destroy callbacks and double-destroy.
- **MangoSlider.luau** — Apple-style slider. 6px glass-material track with colored fill and 28px clean white circular thumb that grows to 34px on drag. Thumb has subtle 0.5px gray border (UIStroke) for definition against any background. FillFrame has UICorner=999 and ClipsDescendants=true to match the pill track shape and prevent white corner artifacts. FillHighlight has no UICorner (parent fillFrame clips). Single outer shadow. Global drag tracking via UserInputService.InputChanged. `isDestroyed` flag guards `InputChanged` callback, `InputEnded` callback, and `Destroy()` against double-destroy; sets `isDragging = false` on destroy.
- **MangoButton.luau** — Apple-style glass pill button. Composes `MangoGlassFrame` with `LightweightMode = true`. Hover via UIScale (1.02, 0.2s Quad Out) with glass opacity thickening (-0.04), press via UIScale (0.97) with touch-point specular shift (Offset 0.06). Directional specular. `isDestroyed` flag guards all 4 input handlers (hover enter/leave, press, release) and `Destroy()` against post-destroy callbacks and double-destroy.
...
- **MangoDropdown.luau** — Apple-style inline expanding dropdown. Glass trigger button with chevron (`"v"` GothamMedium 10 — ASCII for Gotham compatibility) wrapped in a Frame for rotation animation (0↔180°). Item checkmarks use SourceSansBold for reliable Unicode rendering. Uses **inline expansion pattern** (follows Orion/Kavo/Rayfield convention): container has `ClipsDescendants=true`, starts at trigger-only height, tweens height to reveal items panel below trigger. Parent UIListLayout auto-pushes siblings down on open, back up on close. Open animation: 0.25s Back Out (Apple overshoot). Close animation: 0.2s Quad Out (snappy retraction). Panel glass has `ShadowEnabled=false` (would be clipped by container anyway). Per-item hover tween map (`{[Frame]: Tween}`) fixes hover-color-sticks bug — each item's hover only cancels THAT item's tween, not all items globally. Max 5 visible items (scrollable). Click trigger to toggle. Item click selects+closes (single-select) or toggles (multi-select). `SetItems()` closes dropdown before rebuilding. `isDestroyed` guards on item hover, item click, and trigger click handlers.
...
- **MangoCheckbox.luau** — Animated checkbox with UIScale fill (0.5→1.0, 0.2s Back Out). Checkmark label uses `Enum.Font.SourceSansBold` (not GothamBold) for reliable ✓ Unicode rendering across all Roblox platforms. Optional label. UIStroke border color animates. `isDestroyed` guard on click handler prevents post-destroy callbacks.
...
- **MangoDialog.luau** — Centered modal dialog. Overlay fades in (0.2s), dialog scales 0.8→1.0 (0.25s Back Out). Apple alert button styles: default, cancel (bold), destructive (red). Overlay tap dismisses. Dismiss ordering fix: sets `isDestroyed = true` BEFORE calling `OnDismissed()` callback to prevent re-entrancy. Button labels use `TextTruncate = AtEnd`.
- **MangoActionSheet.luau** — Bottom action sheet. Slides up from bottom (0.3s Back Out). Separate cancel button in own glass group (8px below actions). Overlay tap dismisses. Dismiss ordering fix: sets `isDestroyed = true` BEFORE calling `OnDismissed()` callback to prevent re-entrancy. Action labels and cancel label use `TextTruncate = AtEnd`.
...
- **Carousel clip padding**: ClipFrame uses 14px edge padding to accommodate icon shadow overflow (6px beyond icon bounds via ICON_SIZE+6 shadow) and active dots (6px + 3px offset below icon). Demo carouselContainer reduced by 12px with centered anchoring for matching breathing room.
...
- **Dismiss ordering safety**: Set `isDestroyed = true` BEFORE calling `OnDismissed()` callbacks (MangoToast, MangoNotification, MangoDialog, MangoActionSheet) to prevent re-entrancy.
- **isDestroyed guards**: MangoSlider, MangoToggle, MangoButton, MangoCheckbox, and MangoDropdown use `isDestroyed` flags to guard against post-destroy callbacks and double-destroy. MangoButton guards all 4 input handlers (hover enter/leave, press, release). MangoToggle guards `InputBegan` and `SetState()`. MangoSlider guards `InputChanged` and `InputEnded`; also sets `isDragging = false` on destroy. MangoCheckbox guards click handler. MangoDropdown guards blocker click, item hover, and item click handlers.
- **`.Completed:Once()` over `.Completed:Connect()`**: MangoNotification dismiss tween and MangoCarousel scroll tween use `:Once()` to prevent stale/duplicate callbacks accumulating across multiple operations.
...
- **Window pendingDelays cancellation**: `showWindow()` cancels all `pendingDelays` before scheduling new delay threads, preventing animation stacking from rapid show/hide cycles. `hideWindow()` delay threads tracked in `pendingDelays` for cleanup on `Destroy()`. Close button hover tween tracked via `trackTween()`.
...
- **Dropdown inline expansion**: Container has `ClipsDescendants=true` and tweens its height between `triggerHeight` (collapsed) and `triggerHeight + PANEL_GAP + panelHeight` (expanded). Parent UIListLayout pushes siblings automatically. No overlay ScreenGui, no click blocker, no AbsolutePosition tracking. Panel glass `Visible=false` set in `.Completed:Once()` after collapse tween to prevent items showing during collapse. Chevron wrapper Frame rotation tweens 0↔180° for open/close indication.
- **Dropdown per-item hover map**: `itemHoverTweenMap: {[Frame]: Tween}` replaces shared `itemHoverTweens` array. Each item's hover enter/leave only cancels THAT item's tween — fixes bug where moving between items left previous items stuck at hover color. `clearAllItemHovers()` resets all on close.
- **Dropdown SetItems() safety**: Closes dropdown before rebuilding items to prevent stale item connection conflicts.
- **Dropdown in MangoWindow**: Row has `AutomaticSize = Enum.AutomaticSize.Y` so it grows when dropdown expands. Name label uses fixed pixel height `(0.4, 0, 0, 42)` instead of scale `(0.4, 0, 1, 0)` to avoid stretching.
- **Dropdown in MangoForm**: componentWrapper has `AutomaticSize = Enum.AutomaticSize.Y` for expansion propagation through fieldContainer to form container.
